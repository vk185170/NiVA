...
class IMSupportDialog extends ComponentDialog {
    ...
    async confirmProductCategoryStep(step) {
        ...
        if (step.values.version[0] === 'V') {
            if (step.values.version === 'Vision 2x' || step.values.version === 'Vision 3x') {
                ...
                return await step.prompt(CHOICE_PROMPT, {
                    prompt: 'Would you like us to fill a RFP form to get upgraded to Vision 4.x?',
    ...
    async queryStep(step) {
        step.values.product = step.result.value;
        if (step.values.product === 'Yes') {
            await step.context.sendActivity('We have recieved your request for upgrading the Vision');
            endDialog = true;
            return step.endDialog();
        } else if (step.values.product === 'No') {
            endDialog = true;
            return await step.endDialog();
        }
